...
import { Headline as DefaultHeadline } from "./headline-fb";
import { Headline as GoogleHeadline } from "./headline-gg";
...
export default function Formv3() {
  ...
  useEffect(() => {
    if (params && params.temperature) {
      ...
      const redLineText = activeHeadline.find(
        (benefit) => benefit.id === redLineVersion
      )?.text;

      const titleRedLineText = activeHeadline.find(
        (benefit) => benefit.id === redLineVersion
      )?.title;
  ...
  const handleLeadCaptureSubmit = async (data: LeadCaptureSubmitData) => {
    ...
    try {
      ...
      window.location.href = `/quiz-platform/?temperature=${temperatura}&requestId=${encodeURIComponent(
        requestId

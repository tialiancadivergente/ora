...
const leadCaptureFormWithoutNameSchema = leadCaptureFormSchema.omit({
  name: true,
});

type LeadCaptureFormWithoutNameValues = Omit<
  LeadCaptureFormValues,
  "name"
>;
...
export function normalizePhone(
  data: Pick<LeadCaptureFormValues, "ddi" | "whatsapp">
): string {
...
export function LeadCaptureForm({
  ...
  } = useForm<LeadCaptureFormWithoutNameValues>({
    resolver: zodResolver(leadCaptureFormWithoutNameSchema),
    defaultValues: {
      email: defaultValues?.email ?? "",
      ddi: defaultValues?.ddi ?? "+55",
  ...
  const submitHandler: SubmitHandler<
    LeadCaptureFormWithoutNameValues
  > = async (values) => {
    await onSubmit({
      name: "",
      ...values,
